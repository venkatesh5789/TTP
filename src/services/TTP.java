...
public class TTP {
	...
	public TTP() {
		...
		System.out
		.println("Enter Retransmission Timer Interval in milliseconds");
		time = new Time(read.nextLong());
	}

	public void open(InetAddress src, InetAddress dest, int srcPort,
			int destPort, int verbose) throws IOException {
	 ...
	 * 
	 * @param flags
	 *            - The flags list is- 1) SYN 2) ACK 3) FIN 4) EOF
	 * @return
	 */
	...
	private void encapsulateAndSendFragment(byte[] fragment,
			boolean lastFragment) throws IOException {
		...
		System.arraycopy(fragment, 0, headerPlusData, header.length,
				fragment.length);
